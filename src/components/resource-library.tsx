...
import { Link2, Search, X, Globe } from 'lucide-react';
...
export default function ResourceLibrary({ isOpen, onOpenChange }: ResourceLibraryProps) {
  ...
  useEffect(() => {
    const handler = setTimeout(async () => {
        ...
        setIsSearching(true);
        try {
          const results = await searchElsevier(searchQuery);
          setElsevierResults(results);
        } catch (e) {
          console.error("Failed to search Elsevier", e);
          setElsevierResults([]);
        } finally {
    ...
    return () => {
        ...
        setIsSearching(false); // Cancel search if component unmounts or query changes
    };
  }, [searchQuery]);
          ...
          <Card className="w-full h-full flex flex-col border-0 rounded-lg">
              <CardHeader>
                  ...
                  <div className="pt-2 relative">
                    ...
                    <Input
                      placeholder="Buscar en la biblioteca y en Elsevier..."
                      value={searchQuery}
              ...
              <CardContent className="flex-grow min-h-0">
                  <ScrollArea className="h-full pr-4">
                      ...
                      {(isSearching || elsevierResults.length > 0 || searchQuery.length >= 3) && (
                          <>
